...
\begin{itemize}
  ...
  \item wieloklasowa regresja logistyczna.
\end{itemize}
...
Przypatując się wykresom (\ref{fig:fig1}), (\ref{fig:fig2}) możemy zauważyć, że zachodzi zjawisko maskowania --- klasa \verb|versicolor| jest przysłaniona.
...
Po przeanalizowaniu wykresów (\ref{fig:fig3}) i (\ref{fig:fig4}) dochodzimy do wniosku, że w tym przypadku nie występują zjawisko maskowania klas.
...
```{r Wine_boxplot, echo=FALSE, eval=TRUE, fig.cap="Wykresy pudelkowe naszych danych."}
...
Zobaczmy więc jak zmienia się błąd klasyfikacji w zależności od parametru $k$.
...
Jak się okazuje, model ten również przyjmuje $k = 9$ ($\verb|model\$bestTune| = $`r model$bestTune`).
...
Jak widzimy model stuningowany poradził sobie najlepiej. Wyznaczymy teraz dla niego błąd predykcji --- skorzystamy z $5$-krotnej walidacji krzyżowej.
...
Błąd wyniósł `r knn.error.cv$error`.
...
Widzimy, że najlepsze reultaty otrzymujemy dla cp = $0$ --- drzewa nie trzeba w ogóle przycinać.
...
Błąd wyniósł `r decision.tree.error.cv$error`.
...
Powtórzymy teraz ocenę klasyfikacji przy pomocy metody $5$-krotnej walidacji krzyżowej dla stuningowanego klasyfikatora bayesowskiego.
...
Błąd predykcji wynosi `r error.cv$error`. 
...
\caption{Macierze pomylek dla regresji wielorakiej --- wszystkie cechy.}
\label{tab:tab_mlr1}
\end{table}
...
\caption{Macierze pomylek dla wielorakiej regresji --- wybrane cechy.}
\label{tab:tab_mlr2}
\end{table}
...
\caption{Macierze pomylek dla klasyfikatora regresji wielorakiej --- model stuningowany.}
\label{tab:tab_mlr3}
\end{table}
...
Błąd predykcji wynosi `r mlr.error.cv$error`. 

## Podsumowanie

Porównaliśmy ze sobą 4 klasyfikatory. Dla każdego z nich badaliśmy jaki wpływ na dokładność i ch predykcji ma zmiana charaktrystycznych dla nich parametrów czy zmiany w zbiorze uczącym (klasyfikatory trenowliśmy na zbiorach, które albo zawierały wszystkie zmienne, albo te wybrane przez nas, które wprowadzły najlepszy podział na zmienne).

\begin{itemize}
  \item Dla metody $k$ najbliższych sąsiadów najlepsze rezultaty otrzymaliśmy dla $k = 9$ i zbioru uczącego o zawężonej ilości cech.
...
Błędy predykacji dla $5$-krotnej walidacji krzyżowej wyglądają następująco --- patrz tabela. 
...
\hline
KNN & Drzewa decyzyjne & Naiwny klasyfikator bayesowski & Wieloklasowa regresja logistyczna \\
\hline
`r knn.error.cv$error` & `r decision.tree.error.cv$error` & `r error.cv$error` & `r mlr.error.cv$error` \\
\hline
\end{tabular}
}
