...
Teraz zbudujemy klasyfikator oparty na algorytmie SVM. W celu późniejszej wizualizacji obszarów decyzyjnych oraz dla uproszczenia modelu wybierzemy tylko 2 cechu \verb|Alcohol| i \verb|Flavanoids|.

```{r}
wine <- wine %>% select(c(Type, Alcohol, Flavanoids))
```

Korzystająć z walidacji krzyżowej sprawdzimy jak klasyfikator z jądrem liniowym radzi sobie przy różnych wartościach parametru kosztu $C$.
...
Możemy zauważyć, że najlepsze wyniki uzyskujemy dla stosunkowo małej wartości parametru kosztu ($C=0.1$).

Zbadamy teraz jak wyglądają obszary decyzyjne dla poszczegółnych wartości tego parametru.
...
Można zauważyć, że zastosowanie nieliniowych jąder trochę poprawiło dokładność klasyfikatora, natomiast różnice pomiędzy jądrem wielomianowym i gaussowskim są nieznaczne.
...
Mapa ciepła pokazuje, że optymalnymi parametrami są $\sigma=1$ oraz $C=0.1$.
