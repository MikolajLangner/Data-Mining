...
table %>% kbl(caption="Jakis label", format="latex", digits=3) %>% kable_styling(latex_options=c('hold_position', 'scale_down'))
```
...
```{r message=FALSE, fig.height=3, fig.width=6, echo=FALSE, fig.cap=c("1", "2", '3', '4')}
...
```{r variance, echo=FALSE, fig.width=8, fig.cap="Wariancja wyjasniana przez poszczegolne skladowe glowne i wariancja skumulowana.", fig.pos="h"}
...
```{r 2d plot region, echo=FALSE, fig.width=10, fig.height=3.5, fig.cap="Wykresy rozrzutu", fig.pos="h"}
p1 <- ggplot(dat = data.frame(after.pca$x), aes(x=PC1, y=PC2)) + geom_point(aes(color = state$region))

p2 <- ggplot(dat = data.frame(after.pca$x), aes(x=PC1, y=PC2)) + geom_point(aes(color = state$division))
...
cor.matrix %>% kbl(caption="Wartosci macierzy korelacji zmiennych", format="latex", digits=3) %>% kable_styling(latex_options=c('hold_position', 'scale_down'))
```
...
```{r STRESS, warning=FALSE, results='asis'}
d.max <- 16
...
ggplot(data = stress.values, aes(x=seq_along(stress.values), y=stress.values)) + geom_point() + geom_line()
```
