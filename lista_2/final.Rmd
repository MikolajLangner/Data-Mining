...
```{r 2d mds plot, echo=FALSE, fig.cap="Wykres rozrzutu w przestrzeni d = 2"}
mds.2 <- cmdscale(dist.matrix, k = 2)

data.2 <- data.frame(mds.2)
ggplot(data = data.2, aes(x=X1, y=X2)) + geom_text(aes(color = stars$Type, label = rownames(stars)), size = 2, nudge_y = 0.2) +
       geom_point(color = stars$Type)  + labs(x = "MD1", y = "MD2")
```

```{r 2d mds plot2, echo=FALSE, fig.cap="Wykres rozrzutu w przestrzeni d = 2"}
mds.2 <- cmdscale(dist.matrix, k = 2)
